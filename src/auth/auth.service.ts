...
export class AuthService {
    ...
    async generateTokens(payload: { role: number; id: number }): Promise<TokensInterface> {
        return {
            accessToken: await this.jwtService.signAsync(payload, {expiresIn: '15m', secret: process.env.JWT_SECRET}),
            refreshToken:  await this.jwtService.signAsync(payload, {expiresIn: '7d', secret: process.env.JWT_REFRESH_SECRET}),
        };
    }
    ...
    async signUp(data: SignUpDto): Promise<TokensInterface> {
        ...
        const newUser = await this.userService.create({email, name, surname, password: hashedPassword, role: 2, isBanned: false});
    ...
    async refreshTokens(token: string): Promise<TokensInterface> {
        ...
        const decodedToken = this.jwtService.verify(token, {secret: process.env.JWT_REFRESH_SECRET});
