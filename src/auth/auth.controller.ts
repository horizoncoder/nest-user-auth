...
export class AuthController {
    ...
    private setCookies(res: Response, accessToken: string, refreshToken: string): void {
        ...
        res.cookie('accessToken', accessToken, { maxAge: 15 * 60 * 1000, httpOnly: false, secure: false, sameSite: 'strict' }) // 15 min);
        res.cookie('refreshToken', refreshToken, { maxAge: 7 * 60 * 60 * 60 * 1000, httpOnly: true, secure: false, sameSite: 'strict' }) // 7 days);
    }
    ...
    async refreshTokens(@Res() res: Response,@Req() request: Request): Promise<void> {
        try {
